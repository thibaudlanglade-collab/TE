...
def _anonymous_trial_base_url() -> str:
    ...
    Default is empty string → relative URL (`/app/{token}`). That works when
    the landing site and the app live on the same origin (the current prod
    deploy on `synthèse.fr`). Override with `SYNTHESE_APP_URL` only if they
    ever get split across domains again.
    """
    return os.environ.get("SYNTHESE_APP_URL", "").rstrip("/")
