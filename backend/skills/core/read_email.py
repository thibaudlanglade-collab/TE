...
TOOL_SCHEMA = {
    ...
    "description": (
        ...
        "destinataires, pièces jointes. À utiliser après search_emails pour obtenir "
        "les détails d'un email identifié."
    ),
    "when_to_use": [
...
async def execute(input_data: dict, context: Any) -> SkillResult:
    try:
        ...
        if email is None:
            return SkillResult(success=False, data=None, error=f"Email {email_id} introuvable")
